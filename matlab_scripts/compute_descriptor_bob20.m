function [out_data, out_labels] = compute_descriptor_bob20(indices, data)
%% transform molecules
  ...
  for i=1:M
    ...
    for j=i+1:M   
        if (Zs(i) ~= 0 && Zs(j)~= 0)
          ...
          out_data(sample,1,1,idx_zi + nbr_dist_bins*mr_zj + distanceR) = ...
              out_data(sample,1,1,idx_zi + nbr_dist_bins*mr_zj + distanceR) + 1;
          out_data(sample,1,1,idx_zj + nbr_dist_bins*mr_zi + distanceR) = ...
              out_data(sample,1,1,idx_zj + nbr_dist_bins*mr_zi + distanceR) + 1;
        end
    end
 end
end
