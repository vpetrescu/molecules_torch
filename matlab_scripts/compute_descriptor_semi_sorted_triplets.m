function [out_data_triplets, out_labels] = ...
    compute_descriptor_semi_sorted_triplets(indices, data)
...
% bin the indices of the first atom charge
out_data_indices_z1 = zeros(size(data2.X,1), N);
% bin the indices of the second atom charge
out_data_indices_z2 = zeros(size(data2.X,1), N);
% put the distances of the pairs
out_data = zeros(size(data2.X,1), N);
% current fill index for the pair (i,j)
out_data_triplets = zeros(size(data2.X,1), 3*N);
...
for s = 1:size(data2.X,1)
  ...
  for z1 = 1:size(z_values,2)
    for z2 = z1+1:size(z_values,2)
        ...
        out_data_indices_z1(s, index) = z_values(z1);
        out_data_indices_z2(s, index) = z_values(z2);
        out_data(s, index) = (z_values(z1)*z_values(z2))/data2.X(s,z1,z2);
        current_fill_index(offset) = current_fill_index(offset) + 1;
    end
  end
  for i=1:size(bin_offsets,1)-2
    t = out_data(s,bin_offsets(i):bin_offsets(i+1)-1);
    [out_data(s,bin_offsets(i):bin_offsets(i+1)-1), indices] = sort(t);
  end
  out_data_indices_z1(s, :) = out_data_indices_z1(s,:);
  out_data_indices_z2(s, :) = out_data_indices_z2(s,:);
  t = [out_data_indices_z1(s,:); out_data_indices_z2(s,:); out_data(s,:)];
  t = t(:);
  out_data_triplets(s,:) = t;
