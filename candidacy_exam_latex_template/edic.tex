...
For $T$ sampled points with noise, we have $y_t = f(x_t) + \epsilon$, where $\epsilon
\in N(0,\sigma^2)$. The posterior over f, conditioned on the $y1..T-1$ and $x1..T$ in presence of noise is given by
$P(f_(T)| y_{1:T-1},x_{1..T}) = N(u(T),)$, where 
\begin{equation}
...
\subsubsection{Bounding the Information Gain using the spectral }
