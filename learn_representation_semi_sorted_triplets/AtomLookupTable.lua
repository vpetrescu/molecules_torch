local AtomLookupTable, parent = torch.class('nn.AtomLookupTable', 'nn.Module')


function AtomLookupTable:__init(nIndex, ...)
   parent.__init(self)
   local arg = {...}
...
function AtomLookupTable:reset(stdv)
   stdv = stdv or 1
   if nn.oldSeed then
...
function AtomLookupTable:zeroGradParameters()
...
function AtomLookupTable:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:dim() == 1 then
...
function AtomLookupTable:accUpdateGradParameters(input, gradOutput, lr)
   if input:dim() == 1 then
      for i=1,input:size(1) do
         local k = input[i]
...
function AtomLookupTable:updateParameters(learningRate)
...
function AtomLookupTable:type(type)
   self._indices = nil
   self._inputView = nil
...
function AtomLookupTable:scaleUpdateByKey(inputKey)
   -- default is to perform no key-based scalling
   return 1
end

-- we do not need to accumulate parameters when sharing
AtomLookupTable.sharedAccUpdateGradParameters = AtomLookupTable.accUpdateGradParameters
