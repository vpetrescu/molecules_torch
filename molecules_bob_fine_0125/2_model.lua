...
-- 10-class problem
noutputs = 1--trainData.labels:size(2)
...
--   model:add(nn.InputDropout(0))
   ...
   model:add(nn.Linear01(ninputs,nhiddens1))
...
--       model:add(nn.SoftPlus())
   ...
   model:add(nn.Linear01(nhiddens1, nhiddens2))
   if (activation_type == 'Tanh') then
     model:add(nn:Tanh())
  elseif (activation_type == 'ReLU') then
    --model:add(nn:Tanh())
   ...
   model:add(nn.Linear01(nhiddens2, noutputs))
   local answer = io.read()
